...
//init
void easytf::OP_HardSigmoid::init(const Param& param)
{
}
//forward
void easytf::OP_HardSigmoid::forward(const std::map<std::string, std::shared_ptr<Entity>>& bottom, std::map<std::string, std::shared_ptr<Entity>>& top)
{
...
void easytf::OP_HardSigmoid::implement(const float32_t* src, float32_t* dst, const int32_t size)
{
	naive_implement(src, dst, size);
}
void easytf::OP_HardSigmoid::naive_implement(const float32_t* src, float32_t* dst, const int32_t size)
{
	for (int32_t i = 0; i < size; i++)
	{
		dst[i] = std::max(0.0f, std::min(1.0f, src[i] * 0.2f + 0.5f));
	}
}